  ...
  <section class="text-gray-600 body-font">
    <div class="container px-5 py-24 mx-auto">
      <div class="flex flex-col text-center w-full mb-6">
        <h1 class="sm:text-4xl text-3xl font-medium title-font mb-2 text-gray-900">Colors({{ $colors->count() }})</h1>
      </div>
            ...
            @foreach ($colors as $key => $c)
              <tr>
                ...
                <td class="w-10 text-center">
                  <a href="{{ route('admin.colors.edit', $c->id) }}"
                ...
                <td class="w-10 text-center">
                  <form id="{{ 'color-' . $c->id }}" method="POST" action="{{ route('admin.colors.destroy', $c->id) }}">
                    @csrf
                    @method('DELETE')
                    <button type="submit"
